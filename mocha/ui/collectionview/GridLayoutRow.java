...
class GridLayoutRow extends MObject implements Copying<GridLayoutRow> {
	...
	List<Rect> layoutRowAndGenerateRectArray(boolean generateRectArray) {
		...
		GridLayoutSection section = this.getSection();
		GridLayoutInfo layoutInfo = section.getLayoutInfo();

		// properties for aligning
		boolean isHorizontal = layoutInfo.horizontal;
		boolean isLastRow = section.getIndexOfImcompleteRow() == this.getIndex();

		GridLayoutAlignmentOptions.Alignment horizontalAlignment = isLastRow ? section.getRowAlignmentOptions().horizontalLastRow : section.getRowAlignmentOptions().horizontal;

		// calculate space that's left over if we would align it from left to right.
		float leftOverSpace = layoutInfo.dimension;
		if (isHorizontal) {
			leftOverSpace -= section.getSectionMargins().top + section.getSectionMargins().bottom;
		}else {
			leftOverSpace -= section.getSectionMargins().left + section.getSectionMargins().right;
		}
		...
		while (itemIndex < this.itemCount() || isLastRow) {
			...
			if (!this.getFixedItemSize()) {
				...
				nextItemSize = isHorizontal ? section.getItemSize().height : section.getItemSize().width;
			}
		...
		Rect itemFrame = new Rect(Point.zero(), section.getItemSize());
		for (itemIndex = 0; itemIndex < this.getItemCount(); itemIndex++) {
			...
			if (isHorizontal) {
				...
				itemOffset.y += itemFrame.size.height + section.getVerticalInterstice();

				if (horizontalAlignment == GridLayoutAlignmentOptions.Alignment.JUSTIFY) {
					itemOffset.y += interSpacing;
				}
			} else {
				itemFrame.origin.x = itemOffset.x;
				itemOffset.x += itemFrame.size.width + section.getHorizontalInterstice();
